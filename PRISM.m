...
%
% You'll need to adjust the figures yourself if you want them to look
% different.
...
B = 0.1;          % Peak scale, 0 = no structure factor: NOTE: EITHER MAKE REALLY SMALL OR USE FC MODEL for no Structure factor.
b = 900;          % Kuhn Length 
...
if B ~=0
    ...
    hold on
    box on
    title("Structure factor, $$S(q) = \frac{1}{1+\beta c(q)(S_{exv}(q)+S_{flex}(q))}$$","interpreter","latex")
    set(gca,'Xscale', 'log','fontweight','bold','Linewidth',1.6)
    set(gca,'Yscale', 'log')
    xlim([1e-3, 0.9e0])
    ylim([0.5e-2, 1e1])
